...
import { DisplayMeta, Meta, Namespace, References, Collections } from '/imports/model/MetaObject';
...
import { dateToString, stringToDate } from '/imports/data/dateParser';
import { getNextUserFromQueue as getNext } from '/imports/meta/getNextUserFromQueue';
...
import { runScriptBeforeValidation, processValidationScript, runScriptAfterSave } from '/imports/data/scripts';
...
export async function find({ authTokenId, document, displayName, displayType, fields, filter, sort, limit, start, getTotal, withDetailFields, contextUser }) {
	try {
		...
		const collection = Collections[document];
		if (collection == null) {
		...
		if (!isObject(filter) && displayName != null && displayType != null) {
			const displayMeta = DisplayMeta[`${document}:${displayType}:${displayName}`];
...
export async function findByLookup({ authTokenId, document, field, search, extraFilter, start, limit, useChangeUserFilter }) {
	...
	const lookupMetaObject = Meta[fieldObject.document];
	if (lookupMetaObject == null) {
		return errorReturn(`[${document}] Lookup document not found`);
	}

	const lookupCollection = Collections[fieldObject.document];
...
export async function create({ authTokenId, document, data, contextUser, upsert, updateOnUpsert, ignoreAutoNumber = false }) {
	...
	if (Object.keys(cleanedData).length > 0) {
		...
		try {
			...
			logger.error(e, `Error on insert ${Namespace.ns}.${document}: ${e.message}`);
			if (e.code === 11000) {
		...
		if (isEmpty(Namespace.onCreate) === false) {
			const hookData = {
				action: 'create',
				ns: Namespace.ns,
			...
			await BluebirdPromise.mapSeries(urls, async url => {
				try {
					const hookUrl = url.replace('${dataId}', insertedQuery._id).replace('${documentId}', `${Namespace.ns}:${document}`);
		...
		if (emailsToSend.length > 0) {
			const messagesCollection = Collections['Message'];
			const now = DateTime.local().toJSDate();
...
export async function update({ authTokenId, document, data, contextUser }) {
	...
	if (metaObject.ignoreUpdatedAt !== true) {
		...
		if (outdateRecords.length > 0) {
			...
			const historyCollection = Collections[`${document}.History`];
	...
	const updateResults = await BluebirdPromise.mapSeries(existsRecords, async record => {
		...
		try {
			...
			logger.error(e, `Error on update ${Namespace.ns}.${document}: ${e.message}`);
			if (e.code === 11000) {
				return errorReturn(`[${document}] Duplicate key error`);
			}
			return errorReturn(`[${document}] ${e.message}`);
	...
	if (updatedIs.length > 0) {
		if (Namespace.onUpdate != null) {
			...
			await BluebirdPromise.mapSeries(urls, async url => {
				try {
					const hookUrl = url.replace('${dataId}', updatedIs.join(',')).replace('${documentId}', `${Namespace.ns}:${document}`);
					const hookResponse = await fetch(hookUrl, {
						method: 'POST',
						body: JSON.stringify(hookData),
					});
...
export async function deleteData({ authTokenId, document, data, contextUser }) {
	...
	const trashCollection = Collections[`${document}.Trash`];
	...
	const references = References[document];
	...
	if (references?.from != null && isObject(references.from)) {
		const referencesResult = await BluebirdPromise.mapSeries(Object.keys(references.from), async referenceName => {
			...
			const referenceCollection = Collections[referenceMeta.document];
	...
	try {
		...
		logger.error(e, `Error on delete ${Namespace.ns}.${document}: ${e.message}`);
		return errorReturn(`[${document}] Error on delete ${Namespace.ns}.${document}: ${e.message}`);
	}

	if (Namespace.onDelete != null) {
		const hookData = {
			action: 'delete',
			ns: Namespace.ns,
			documentName: document,
		...
		await BluebirdPromise.mapSeries(urls, async url => {
			try {
				const hookUrl = url.replace('${dataId}', idsToDelete.join(',')).replace('${documentId}', `${Namespace.ns}:${document}`);
				const hookResponse = await fetch(hookUrl, {
					method: 'POST',
					body: JSON.stringify(hookData),
				});
...
export async function relationCreate({ authTokenId, document, fieldName, data, preview = false, contextUser }) {
	...
	const reverseLookupCollection = Collections[Meta[relation.document].fields[relation.reverseLookup].document];
	...
	const createResults = await BluebirdPromise.mapSeries(relationsIds, async ({ lookup, reverseLookup }) => {
		const getLookupId = async () => {
			if (relation.lookupField != null) {
				const lookupData = await Collections[field.document].findOne(lookup);
		...
		if (sendEmail === true) {
			if (emailData[Meta[relation.document].fields[relation.reverseLookup].document] == null) {
				emailData[Meta[relation.document].fields[relation.reverseLookup].document] = [];
			}
			if (reverseLookupCollection && _find(emailData[Meta[relation.document].fields[relation.reverseLookup].document], { _id: reverseLookup }) == null) {
				...
				const populatedData = await populateLookupsData({
					documentName: Meta[relation.document].fields[relation.reverseLookup].document,
				...
				emailData[Meta[relation.document].fields[relation.reverseLookup].document].push(populatedData);
			}
		}
	...
	if (sendEmail === true && size(get(emailData, relation.document)) > 0) {
		...
		const messageCollection = Collections['Message'];
	...
	if (save != null && isArray(save) === true && size(save) > 0) {
		...
		const saveRelations = async (relations, contactId, parentObj) =>
			BluebirdPromise.mapSeries(relations, async saveObj => {
				...
				if (Meta[saveObj]?.fields?.contact?.isList === true) {
