...
export async function addUser({ authTokenId, document, ids, users }) {
	...
	const updateResults = await Promise.all(
		validateResult.data.map(async newUser => {
			...
			try {
				await Collections[document].updateMany(query, update);
				return { success: true };
			} catch (e) {
				return {
					success: false,
...
export async function replaceUser({ authTokenId, document, ids, from, to }) {
	...
	try {
		await Collections[document].updateMany(query, removeUpdate);
		return { success: true };
	} catch (e) {
		return {
			success: false,
...
export async function countInactive({ authTokenId, document, ids }) {
	...
	try {
		const count = await Collections[document].countDocuments(query);
		return {
			success: true,
			count,
		};
...
export async function setQueue({ authTokenId, document, ids, queue }) {
	...
	const updateResults = await Promise.all(
		ids.map(async id => {
			...
			try {
				await Collections[document].updateOne(query, update);
				return { success: true };
			} catch (e) {
				return {
					success: false,
