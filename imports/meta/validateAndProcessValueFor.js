...
export async function validateAndProcessValueFor({ meta, fieldName, value, actionType, objectOriginalValues, objectNewValues, idsToUpdate }) {
	...
	if (value && field.isUnique === true && field.type !== 'autoNumber') {
		...
		const collection = Collections[meta.name];
		...
		switch (field.type) {
			...
			case 'composite':
				...
				if (field.compositeType === 'reference') {
					const referenceMeta = Meta[field.objectRefId];
					...
					const referenceDataValidationResults = await Promise.all(
						Object.keys(value).map(async key => {
							...
							const validationResult = await validateAndProcessValueFor({ referenceMeta, key, subValue, actionType, value, value, idsToUpdate });
			...
			case 'inheritLookup':
				...
				const lookupCollection = Collections[field.document];
