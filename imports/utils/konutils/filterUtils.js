...
export const filterUtils = {
	parseConditionValue(condition, field, req, subTermPart) {
		let group;
	...
	validateOperator(condition, field, subTermPart) {
		let e;
		if (field.type === 'lookup' && subTermPart !== '._id' && subTermPart.indexOf('.') !== -1) {
			const meta = Meta[field.document];
			if (!meta) {
	...
	parseFilterObject(filter, metaObject, req) {
	...
	parseDynamicData(filter, keyword, data) {
		if (!filter) {
			return;
		}
