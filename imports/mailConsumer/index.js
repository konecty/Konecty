...
async function sendEmail(record) {
	...
	if (isObject(get(Namespace, 'emailServers')) && get(Namespace, `emailServers.${record.server}.useUserCredentials`) === true) {
		user = await Collections['User'].findOne(record._user[0]._id, {
	...
	if ((!record.from || isEmpty(record.from)) && get(record, '_user.length', 0) > 0) {
		user = await Collections['User'].findOne(record._user[0]._id, { projection: { name: 1, emails: 1 } });
	...
	if (!record.to) {
		...
		await Collections['Message'].update({ _id: record._id }, { $set: { status: 'Falha no Envio', error: err } });
				...
				try {
					...
					if (get(response, 'accepted.length') > 0) {
						if (record.discard === true) {
							await Collections['Message'].deleteOne({ _id: record._id });
						} else {
							await Collections['Message'].updateOne({ _id: record._id }, { $set: { status: record.sentStatus || 'Enviada' } });
						}
					...
					err.host = serverHost || record.server;
					await Collections['Message'].updateOne({ _id: record._id }, { $set: { status: 'Falha no Envio', error: err } });

					return errorReturn(err);
				}
			} else {
				logger.error(mail, `📧 There are no mail server configured, Email NOT sent`);
				await Collections['Message'].updateOne({ _id: record._id }, { $set: { status: 'Falha no Envio', error: 'There are no mail server configured, Email NOT sent' } });

				return errorReturn('There are no mail server configured, Email NOT sent');
			}
		} else {
			logger.error(mail, `📧 No address to send e-mail to.`);
			await Collections['Message'].updateOne({ _id: record._id }, { $set: { status: 'Falha no Envio', error: 'No address to send e-mail to.' } });
			return errorReturn('No address to send e-mail to.');
		}
	}
...
async function send(record) {
	...
	if (/.+\.hbs$/.test(record.template) === true) {
		...
		const templateRecord = await Collections['Template'].findOne({ _id: record.template }, { projection: { subject: 1 } });

		if (templateRecord == null) {
			logger.error(`Template ${record.template} not found`);
			await Collections['Message'].updateOne({ _id: record._id }, { $set: { status: 'Falha no Envio', error: { message: `Template ${record.template} not found` } } });
	...
	await Collections['Message'].updateOne({ _id: record._id }, { $set: { body: record.body, subject: record.subject } });
	return sendEmail(record);
}
...
async function consume() {
	...
	const mailCount = await Collections['Message'].countDocuments(query);
	...
	await BluebirdPromise.each(range(0, mailCount), async () => {
		const updatedRecords = await Collections['Message'].findOneAndUpdate(query, update, options);
