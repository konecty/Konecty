...
export function registerMetaDocumentTool(server: McpServer, deps: AdminDeps): void {
	registerMcpTool(server, 
		'meta_document_upsert',
		{
			description:
				'Creates or updates a document metadata object. Returns: save summary in content.text and { success, result } in structuredContent.',
		...
		async ({ id, document }) => {
			const result = await proxyMetaUpsert(deps.user(), id, document);
			...
			const text = appendNextSteps(
				...
				['Run meta_read to verify persisted metadata fields.'],
			);
