		...
		async (acc, fieldName) => {
			...
			if (field.type === 'lookup' && data[fieldName] && fields[fieldName]) {
				...
				if (field.isList === true) {
					...
					if (ids.length > 0) {
						acc[fieldName] = await MetaObject.Collections[field.document].find({ _id: { $in: ids } }, options).toArray();
					}
				} else {
					acc[fieldName] = await MetaObject.Collections[field.document].findOne({ _id: data[fieldName]._id }, options);
				}
			}
			return acc;
		},
