...
async function getDescriptionAndInheritedFieldsToUpdate({ record, metaField, meta, dbSession }) {
	...
	if (isArray(metaField.inheritedFields) && metaField.inheritedFields.length > 0) {
		...
		for await (const lookupField of lookupFields) {
			const keysToFind = []
				.concat(lookupField.descriptionFields || [], lookupField.inheritedFields || [])
				.map(getFieldNamesOfPaths)
				.join();
			...
			const lookupRecord = await Collection.find(
				{
					_id: {
						$in: []
							.concat(record[lookupField.name])
							.map(v => v?._id)
							.filter(Boolean),
					},
				},
				{ projection, session: dbSession },
			).toArray();
...
export default async function updateLookupReference(metaName, fieldName, field, record, relatedMetaName, dbSession) {
	...
	try {
		...
		if (updateResult.modifiedCount > 0) {
			...
			await updateLookupReferences(
				metaName,
				modified.map(m => m._id),
				updateData,
				dbSession,
			);
		}
