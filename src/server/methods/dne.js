...
import { unicodeSortArrayOfObjectsByParam } from 'utils';
...
const init = async () => {
	...
	registerMethod('DNE_CEP_List', async cep => {
	...
	registerMethod('DNE_City_List', async (state, city) => {
		const cityCollection = db.collection('utils.address.BRA.city');
	...
	registerMethod('DNE_District_List', async (state, city, district) => {
	...
	registerMethod('DNE_Place_List', async (state, city, district, place, number, limit) => {
		const placeCollection = db.collection('utils.address.BRA.place');
