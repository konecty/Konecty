		...
		@param data
			...
			,
				...
				map:
					contact: 'contact'
	*/
	registerMethod('process:submit', 'withUser', async request => {
		const response = {
			success: true,
			processData: {},
			errors: [],
		...
		await Promise.all(
			request.data.some(async piece => {
				...
				response.processData = { ...response.processData, ...piecesReturn[piece.name].processData };
	...
	registerMethod('process:generic', 'withUser', async (document, name, request) => {
		const response = {
			success: true,
			processData: {},
			errors: [],
	...
	registerMethod('process:campaignTarget', 'withUser', async request => {
		const response = {
			success: true,
			processData: {},
			errors: [],
	...
	registerMethod('process:opportunity', 'withUser', async request => {
		let createRequest;
		let opportunity;
		let opportunityId;
		const response = {
		...
		if (size(get(record, 'data')) > 0) {
			...
			if (request.product) {
				...
				await Models.Product.find(productFilter).forEach(product => {
			...
			createRequest.data = { ...createRequest.data, ...request };
	...
	registerMethod('process:message', 'withUser', async request => {
		const response = {
			success: true,
			processData: {},
			errors: [],
	...
	registerMethod('process:activity', 'withUser', async request => {
		const response = {
			success: true,
			processData: {},
			errors: [],
	...
	*/
	registerMethod('process:contact', 'withUser', async function (request, options) {
		...
		if (request.extraFields) {
			contactData = { ...contactData, ...request.extraFields };
		}
