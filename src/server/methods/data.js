...
import { registerMethod, callMethod } from 'utils/methods';
...
const processValidationScript = async function (validationScript, validationData, fullData, context) {
	...
	if (validationData) {
		for (const validationField in validationData) {
...
const init = () => {
	...
	*/
	registerMethod('data:find:byId', 'withUser', 'withAccessForDocument', async function (request) {
		...
		const options = { projection: ensureReadConditionsFields(fields, metaObject, this.access, emptyFields) };
	...
	*/
	registerMethod('data:find:byLookup', 'withUser', 'withAccessForDocument', async function (request) {
				...
				if (lookupField) {
					...
					if (isString(request.search) && request.search.length > 0) {
						...
						if (['number', 'autoNumber', 'money'].includes(lookupField.type)) {
							...
							for (const addressField of ['country', 'state', 'city', 'place', 'number', 'postalCode', 'district', 'placeType', 'complement']) {
								const c = {};
		...
		if (isArray(field.inheritedFields)) {
			for (const inheritedField of field.inheritedFields) {
	...
	registerMethod('data:populate:detailFieldsInRecord', 'withUser', 'withAccessForDocument', async function (request) {
		...
		const populateDetailFields = async function (field, value, parent) {
			...
			if (has(record, 'data.0')) {
				for (const recordKey in record.data[0]) {
		...
		for (const fieldName in request.record) {
	...
	registerMethod(
		...
		async function (request) {
			...
			if (get(request, 'data._user', undefined)) {
				...
				for (const newUser of request.data._user) {
			...
			for (const fieldName in request.data) {
			...
			if (meta.scriptBeforeValidation) {
				...
				request.data = { ...request.data, ...scriptBeforeValidationData };
			}
			...
			if (meta.validationScript) {
				const validation = await processValidationScript(meta.validationScript, meta.validationData, { ...request.data, ...newRecord }, context);
		...
		async function (request) {
			...
			if (outOfDateQuery.length > 0) {
				...
				if (outOfDateRecords.length > 0) {
					...
					for (const outOfDateRecord of outOfDateRecords) {
			...
			const validateAndUpdateRecords = async records => {
				...
				if (meta.scriptBeforeValidation) {
					...
					const scriptBeforeValidationData = await runScriptBeforeValidation(
						...
						{ ...records[0], ...request.data.data, ...validatedData },
						context,
						extraData,
					);
					bodyData = { ...request.data.data, ...scriptBeforeValidationData };
				} else {
					bodyData = { ...request.data.data };
				}
				...
				// If meta includes validationScript, run it in a sandboxed environment
				if (meta.validationScript) {
					const validation = await processValidationScript(meta.validationScript, meta.validationData, { ...records[0], ...validatedData }, context);
	...
	*/
	registerMethod('data:delete', 'withUser', 'withAccessForDocument', 'ifAccessIsDeletable', 'withMetaForDocument', 'withModelForDocument', async function (request) {
		...
		if (isObject(references) && isObject(references.from)) {
			for (const referenceMetaName in references.from) {
				// Get model
				...
				if (referenceModel) {
					...
					for (const referenceFieldName in referenceMeta) {
						const condition = {};
					...
					if (referenceConditions.length > 0) {
						...
						if (referenceResult != null) {
							...
							for (id of idsToVerifyRelations) {
								...
								for (const referenceCondition of referenceConditions) {
	...
	*/
	registerMethod('data:relation:create', 'withUser', 'withAccessForDocument', async function (request) {
		...
		for (const metaRelation of field.relations) {
		...
		for (let lookup of request.data.lookups) {
			for (const reverseLookup of request.data.reverseLookups) {
				// Clone extra data
				let newData;
				let populateFields;
				...
				if (sendEmail && request.preview) {
					...
					if (isArray(result.data)) {
						...
						if (sendEmail) {
							...
							populateFields = {};
							populateFields[relation.lookup] = 1;
							result.data.forEach(async resultData => {
		...
		if (sendEmail && size(get(emailData, relation.document)) > 0) {
			...
			if (has(relation, 'emailConf.extraData')) {
				for (const fieldEmail in relation.emailConf.extraData) {
			...
			if (has(Meta, 'Message.fields.opportunity.descriptionFields') && has(relation, 'emailConf.opportunity')) {
				...
				copyObjectFieldsByPathsIncludingIds(objectByString(emailData, relation.emailConf.opportunity), emailOpportunityData, Meta.Message.fields.opportunity.descriptionFields);
			...
			if (request.preview) {
				return await renderTemplate(messageData.template, { message: { _id: Random.id() }, ...emailData });
			}
	...
	registerMethod('data:lead:save', 'withUser', async function (request) {
		...
		if (phoneSent.length > 0) {
			if (size(get(contact, 'phone.length')) > 0) {
				...
				phoneSent.forEach(async leadPhone => {
					if (
