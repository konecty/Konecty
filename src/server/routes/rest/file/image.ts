...
async function imageApiFn(
	...
	if (getStyleRegex.test(incomingPath)) {
		...
		if (['full', 'thumb', 'wm'].includes(style)) {
			...
			const resolvedFileName = style === 'full' ? fileName : `${filePathWithoutExtension(fileName)}.jpeg`;

			const destination = path.join(...([document, recordId, fieldName, dirEnum[style], resolvedFileName].filter(Boolean) as string[]));
			return sendFile(reply, req.url, destination);
		}
	}
	...
	if (legacyUrlRegex.test(incomingPath)) {
		...
		const getImagePath = () => {
			...
			if (maxDimension <= thumbnailSize) {
				return path.join(document, recordId, fieldName, 'thumbnail', `${filePathWithoutExtension(fileName)}.jpeg`);
			}
			if (preprocess != null) {
				return path.join(document, recordId, fieldName, 'watermark', `${filePathWithoutExtension(fileName)}.jpeg`);
			}
