...
const validateRequest = function(request, access) {
	...
	for (let user of Array.from(request.users)) {
		if ((_.isObject(user) !== true) || (_.isString(user.id) !== true)) {
			return new Meteor.Error('internal-error', `[${request.document}] Each user must be and Object with an [_id] as String`, {request});
		}
	}
	...
	for (let id of Array.from(request.ids)) {
...
/* Add users
	...
	@param users
*/
Meteor.registerMethod('changeUser:add', 'withUser', 'withAccessForDocument', 'withMetaForDocument', 'withModelForDocument', function(request) {
	...
	const resultOfValidation = metaUtils.validateAndProcessValueFor(this.meta, '_user', request.users, 'update', this.model, {}, {}, request.ids);
...
/* Remove users
	...
	@param users
*/
Meteor.registerMethod('changeUser:remove', 'withUser', 'withAccessForDocument', 'withMetaForDocument', 'withModelForDocument', function(request) {
	...
	try {
		this.model.updateOne(query, update);
	} catch (e) {
		return e;
	}
...
/* Define users
	...
	@param users
*/
Meteor.registerMethod('changeUser:define', 'withUser', 'withAccessForDocument', 'withMetaForDocument', 'withModelForDocument', function(request) {
...
/* Replace users
	...
	@param users
*/
Meteor.registerMethod('changeUser:replace', 'withUser', 'withAccessForDocument', 'withMetaForDocument', 'withModelForDocument', function(request) {
	let e;
	...
	const resultOfValidation = metaUtils.validateAndProcessValueFor(this.meta, '_user', [request.to], 'update', this.model, {}, {}, request.ids);
	if (resultOfValidation instanceof Error) {
		return resultOfValidation;
	}
...
/* Count inactive users
	...
	@param ids
*/
Meteor.registerMethod('changeUser:countInactive', 'withUser', 'withAccessForDocument', 'withMetaForDocument', 'withModelForDocument', function(request) {
...
/* Remove inactive users
	...
	@param ids
*/
Meteor.registerMethod('changeUser:removeInactive', 'withUser', 'withAccessForDocument', 'withMetaForDocument', 'withModelForDocument', function(request) {
	if (this.access.removeInactiveUser !== true) {
		return new Meteor.Error('internal-error', `[${request.document}] You don't have permission to remove inactive users`);
	}
...
/* Set queue and user
	...
	@param queue
*/
Meteor.registerMethod('changeUser:setQueue', 'withUser', 'withAccessForDocument', 'withMetaForDocument', 'withModelForDocument', function(request) {
	...
	for (var id of Array.from(request.ids)) {
