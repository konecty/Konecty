...
Meteor.registerMethod('merge:simulate', 'withUser', 'withAccessForDocument', 'withMetaForDocument', 'withModelForDocument', function(request) {
	...
	if (request.ids.indexOf(request.targetId) === -1) {
		return new Meteor.Error('internal-error', 'A propriedade [targetId] deve ser um dos valores informados na propriedade [ids]', {request});
	}
	...
	const removeDuplicatedValues = function(values, directValues) {
		...
		for (let index1 = 0; index1 < values.length; index1++) {
			...
			let equals = false;
			for (let start = index1+1, index2 = start, end = values.length, asc = start <= end; asc ? index2 < end : index2 > end; asc ? index2++ : index2--) {
	...
	for (let fieldName in meta.fields) { const field = meta.fields[fieldName]; if (!Array.from(excludedTypes).includes(field.type) && !Array.from(excludedNames).includes(field.name)) { processField(field); } }
...
Meteor.registerMethod('merge:save', 'withUser', 'withAccessForDocument', 'withMetaForDocument', 'withModelForDocument', function(request) {
	...
	if (request.data._merge == null) { request.data._merge = []; }
	...
	for (let referenceDocumentName in (References[request.document] != null ? References[request.document].from : undefined)) {
		const referenceFields = (References[request.document] != null ? References[request.document].from : undefined)[referenceDocumentName];
