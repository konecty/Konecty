...
import { rpad } from '@konecty/utils';
...
const app = {
	...
	post(path, cb) {
		expressApp.post(path, function (req, res, next) {
	...
	put(path, cb) {
		expressApp.put(path, function (req, res, next) {
			if (req.query == null && req?.params?.query != null) {
				req.query = req.query;
			}
			cb(req, res, next);
	...
	del(path, cb) {
		expressApp.delete(path, function (req, res, next) {
			if (req.query == null && req?.params?.query != null) {
				req.query = req.query;
			}
			cb(req, res, next);
