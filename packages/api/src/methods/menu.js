...
import { MetaObject } from '@konecty/metadata';
...
const init = () => {
	...
	*/
	registerMethod('menu', 'withUser', async function (request) {
		...
		await MetaObject.find({ type: { $nin: ['namespace', 'access'] } }, { sort: { _id: 1 } }).forEach(function (metaObject) {
			let value;
		...
		await MetaObject.find({ _id: { $in: accesses } }).forEach(function (metaObject) {
			metaObject.namespace = Namespace.ns;
