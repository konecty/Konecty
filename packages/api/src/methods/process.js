...
import { Models, Meta, Namespace } from '@konecty/metadata';

import { callMethod, registerMethod, methodExists } from '@konecty/utils/methods';
import { getNextUserFromQueue } from '@konecty/utils/meta';
import { getObjectPathAgg, setObjectByPath } from '@konecty/utils';

const findCampaign = async function (search) {
	let filter;
	if (!search) {
		return null;
	}
	...
	const record = await callMethod('data:find:all', {
		document: 'Campaign',
		filter: {
			conditions: [filter],
		},
...
const findChannel = async function (search) {
	if (!search) {
		return null;
	}
...
const init = () => {
		...
		@param data
			...
			,
				...
				map:
					contact: 'contact'
	*/
	registerMethod('process:submit', 'withUser', async function (request) {
		const response = {
			success: true,
			processData: {},
			errors: [],
		...
		if (!isArray(request.data)) {
			return new Error('[internal-error] Invalid payload');
		}

		const piecesReturn = {};

		await Promise.all(
			request.data.some(async function (piece) {
				if (!piece.name) {
					return false;
				}
				...
				if (piece.map) {
					for (let field in piece.map) {
						...
						const lookupValue = getObjectPathAgg(response.processData, lookup);
						if (lookupValue) {
							setObjectByPath(params, field.split('.'), lookupValue);
						}
					}
				}
				...
				if (methodExists([`process:${piece.name}`])) {
					piecesReturn[piece.name] = await callMethod(`process:${piece.name}`, params, options);
				} else if (piece.document) {
					piecesReturn[piece.name] = await callMethod('process:generic', piece.document, piece.name, params, options);
				} else {
					response.success = false;
					response.errors = [new Error('[process-invalid-piece] Invalid generic piece, no document specified.')];
					return true;
				}
				...
				response.processData = Object.assign({}, response.processData, piecesReturn[piece.name].processData);
	...
	registerMethod('process:generic', 'withUser', async function (document, name, request) {
		const response = {
			success: true,
			processData: {},
			errors: [],
	...
	registerMethod('process:campaignTarget', 'withUser', async function (request) {
		const response = {
			success: true,
			processData: {},
			errors: [],
		...
		const campaign = await findCampaign(request.campaign);
		if (campaign) {
	...
	registerMethod('process:opportunity', 'withUser', async function (request) {
		...
		let record = await callMethod('data:find:all', {
		...
		if (size(get(record, 'data')) > 0) {
			...
			if (Namespace.alertUserOnExistingOpportunity) {
				const date = new Date();
				const users = await Models['User'].find({ _id: { $in: map(opportunity._user, '_id') } }).toArray();
			...
			if (request.product) {
				...
				await Models['Product'].find(productFilter).forEach(function (product) {
			...
			const source = await findChannel(request.source);
			if (source) {
				request.source = source;
			}

			const channel = await findChannel(request.channel);
			if (channel) {
				request.channel = channel;
			}

			createRequest.data = Object.assign({}, createRequest.data, request);
		...
		if (request.product) {
			...
			if (productsList) {
				await Promise.all(
					productsList.map(async function (productId) {
						record = await callMethod('data:find:all', {
						...
						if (size(get(record, 'data')) === 0) {
							...
							const saveProductResult = await callMethod('data:create', createRequest);
	...
	registerMethod('process:message', 'withUser', async function (request) {
		const response = {
			success: true,
			processData: {},
			errors: [],
	...
	registerMethod('process:activity', 'withUser', async function (request) {
		const response = {
			success: true,
			processData: {},
			errors: [],
	...
	*/
	registerMethod('process:contact', 'withUser', async function (request, options) {
		let record, result;
		const context = this;
		// meta = @meta
		...
		if (codeSent === false && emailSent.length === 0 && phoneSent.length === 0) {
			response.success = false;
			response.errors = [new Error('[process-contact-validation] É obrigatório o preenchimento de ao menos um dos seguintes campos: code, email e telefone.')];
		...
		if (request.extraFields) {
			contactData = Object.assign({}, contactData, request.extraFields);
		}
		...
		if (userFilter) {
			...
			if (!contactUser && has(request, 'queue._id')) {
				userQueue = await getNextUserFromQueue(request.queue._id, this.user);
		...
		if (contact == null) {
			...
			const {
				fields: { status, type },
			} = await callMethod('document', { document: 'Contact' });
			...
			const updateRequest = {
				document: 'Contact',
				data: {
					ids: [{ _id: contact._id, _updatedAt: { $date: DateTime.fromJSDate(contact._updatedAt).toISO() } }],
					data: contactData,
				},
			};
		...
		// 			saveResult = await callMethod 'data:create', createRequest
